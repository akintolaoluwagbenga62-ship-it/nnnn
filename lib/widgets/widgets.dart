...
class BAvatar extends StatelessWidget {
  ...
  Color get _color {
    final colors = [const Color(0xFF7C3AED), const Color(0xFF1DA1F2), const Color(0xFF00BA7C), const Color(0xFFFF4500), const Color(0xFFF59E0B)];
  ...
  Widget build(BuildContext context) {
    if (isAnonymous) {
      return Stack(children: [
        Container(
          ...
          decoration: BoxDecoration(color: BColor.bg2, shape: BoxShape.circle, border: Border.all(color: BColor.border)),
    ...
    return Stack(children: [
      Container(
        ...
        child: Center(child: Text(name.isEmpty ? '?' : name[0].toUpperCase(), style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: size * 0.4))),
      ),
      if (faceVerified) Positioned(
        bottom: 0, right: 0,
        child: Container(
          width: size * 0.32, height: size * 0.32,
          decoration: BoxDecoration(color: BColor.green, shape: BoxShape.circle, border: Border.all(color: BColor.bg, width: 1.5)),
          child: Center(child: Icon(Icons.verified, color: Colors.white, size: size * 0.18)),
        ),
      ),
    ]);
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Text('Ghost Mode active — you\'re invisible  •  ${_fmt(timeLeft)} left', style: const TextStyle(color: BColor.textSub, fontSize: 12))),
...
class MoodChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final col = mood == 'all' ? BColor.text : BColor.mood(mood);
    return GestureDetector(
      onTap: onTap,
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? col.withOpacity(0.15) : Colors.transparent,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: selected ? col : BColor.border),
        ),
        child: Text(_labels[mood] ?? mood, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: selected ? col : BColor.muted)),
      ),
    );
  }
}
...
class BChatListItem extends StatelessWidget {
        ...
        child: Row(children: [
          ...
          isGroup
              ? Container(
                  width: 48, height: 48,
                  decoration: BoxDecoration(color: BColor.bg2, shape: BoxShape.circle, border: Border.all(color: BColor.border)),
                  child: Center(child: Text(_groupEmoji(chat.topic), style: const TextStyle(fontSize: 22))),
                )
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Row(children: [
                Text(isGroup ? (chat.name ?? 'Group') : (otherUser?.name ?? 'User'), style: const TextStyle(fontWeight: FontWeight.w700, color: BColor.text, fontSize: 15), overflow: TextOverflow.ellipsis),
                if (chat.isAiModerated) ...[const SizedBox(width: 4), const Icon(Icons.smart_toy, color: BColor.chill, size: 13)],
                if (isGroup) ...[const SizedBox(width: 4), Text('${formatCount(chat.memberCount)}', style: const TextStyle(fontSize: 11, color: BColor.muted))],
              ])),
              Text(formatTime(chat.lastMessageAt), style: const TextStyle(fontSize: 11, color: BColor.muted)),
            ]),
            const SizedBox(height: 3),
            Row(children: [
              Expanded(child: Text(chat.lastMessage, style: const TextStyle(fontSize: 13, color: BColor.muted), overflow: TextOverflow.ellipsis, maxLines: 1)),
  ...
  String _groupEmoji(String topic) {
          ...
          if (i == 0) {
            return Padding(
              ...
              child: Column(children: [
                Stack(children: [
                  Container(width: 54, height: 54, decoration: BoxDecoration(color: BColor.bg2, shape: BoxShape.circle, border: Border.all(color: BColor.border))),
                  Positioned(bottom: 0, right: 0, child: Container(width: 18, height: 18, decoration: const BoxDecoration(color: BColor.text, shape: BoxShape.circle), child: const Icon(Icons.add, color: BColor.bg, size: 12))),
                ]),
          ...
          return Padding(
            ...
            child: Column(children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: const LinearGradient(colors: [BColor.logoA, BColor.logoB]),
              ...
              const SizedBox(height: 4),
              SizedBox(width: 54, child: Text(u.name.split(' ').first, style: const TextStyle(fontSize: 10, color: BColor.muted), overflow: TextOverflow.ellipsis, textAlign: TextAlign.center)),
            ]),
          );
        },
      ),
...
class PasswordStrengthBar extends StatelessWidget {
  ...
  Color get color {
    ...
    if (strength == 4) return const Color(0xFF84CC16);
    return BColor.green;
  }
